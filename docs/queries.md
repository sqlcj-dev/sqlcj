...
- Query order inside a file is preserved.
- A query name must be unique across every configured query source, because it
  becomes a generated Java class name.
...
Each query generates one final class in the configured `java.package`, written
to the package directory under `java.out` and named after the query.

- The class has a single constructor taking a
  `dev.sqlcj.runtime.QueryExecutor`.
- The generated method name is the class name with a lower-case first character.
- A `:one` or `:many` query also generates a nested `public record` named
  `<ClassName>Result` whose components follow the selected-column order, plus a
  private `RowMapper` that reads each column by its one-based position.
...
public final class CreateAuthor {

    private final QueryExecutor executor;

    public CreateAuthor(QueryExecutor executor) {
    ...
    private static final RowMapper<CreateAuthorResult> ROW_MAPPER =
...
one generated class are disambiguated in their SQL order. Those rules, and the
rejection of two queries whose generated files would collide, are documented in
[Generated Java Names](configuration.md#generated-java-names).

The generated classes are executed through `dev.sqlcj.runtime.JdbcQueryExecutor`
on a `DataSource` or on a caller-owned `Connection`. Connection ownership,
transaction control, and exception translation are documented in
...
- `JavaCodeGeneratorTest` covers the generated class, constructor, result
  record, row mapper, method signature, and compilation of the generated source.
